/**
 * ODBC core definitions.
 *
 * Cross-platform D translation of the ODBC 4.0 `sql.h` header (the main
 * include for the ODBC Core functions). These bindings always expose the full
 * ODBC 4.0 (`ODBCVER == 0x0400`) surface.
 *
...
enum SQL_DESC_CHARACTER_SET_CATALOG = 1018;
enum SQL_DESC_CHARACTER_SET_SCHEMA = 1019;
enum SQL_DESC_CHARACTER_SET_NAME = 1020;
enum SQL_DESC_COLLATION_CATALOG = 1015;
enum SQL_DESC_COLLATION_SCHEMA = 1016;
enum SQL_DESC_COLLATION_NAME = 1017;
enum SQL_DESC_USER_DEFINED_TYPE_CATALOG = 1026;
enum SQL_DESC_USER_DEFINED_TYPE_SCHEMA = 1027;
enum SQL_DESC_USER_DEFINED_TYPE_NAME = 1028;
...
enum SQL_VARIANT_TYPE = SQL_UNKNOWN_TYPE;
enum SQL_UDT = 17;
enum SQL_ROW = 19;
enum SQL_ARRAY = 50;
enum SQL_MULTISET = 55;

/* One-parameter shortcuts for date/time data types */
enum SQL_TYPE_DATE = 91;
enum SQL_TYPE_TIME = 92;
enum SQL_TYPE_TIMESTAMP = 93;
enum SQL_TYPE_TIME_WITH_TIMEZONE = 94;
enum SQL_TYPE_TIMESTAMP_WITH_TIMEZONE = 95;
...
enum SQL_CODE_TIME_WITH_TIMEZONE = 4;
enum SQL_CODE_TIMESTAMP_WITH_TIMEZONE = 5;
...
/* SQL_OJ_CAPABILITIES bitmasks (outer join) */
...
/*
 * Core ODBC entry points. On Windows these use the `__stdcall` convention
...
SQLRETURN SQLBindParam(SQLHSTMT StatementHandle, SQLUSMALLINT ParameterNumber,
        ...
        SQLSMALLINT ParameterScale, SQLPOINTER ParameterValue, SQLLEN* StrLen_or_Ind);

SQLRETURN SQLCancel(SQLHSTMT StatementHandle);

// ODBC 3.8 addition. Excluded under `ODBC35` (the iODBC baseline on macOS),
// which caps the surface at ODBC 3.5.
version (ODBC35)
{
}
else
...
// ODBC 3.8 addition. Excluded under `ODBC35`.
version (ODBC35)
{
}
else
    SQLRETURN SQLCompleteAsync(SQLSMALLINT HandleType, SQLHANDLE Handle,
...
SQLRETURN SQLGetConnectOption(SQLHDBC ConnectionHandle, SQLUSMALLINT Option,
        SQLPOINTER Value);
...
SQLRETURN SQLGetDescRec(SQLHDESC DescriptorHandle, SQLSMALLINT RecNumber,
        SQLCHAR* Name, SQLSMALLINT BufferLength, SQLSMALLINT* StringLengthPtr,
        SQLSMALLINT* TypePtr, SQLSMALLINT* SubTypePtr, SQLLEN* LengthPtr,
        SQLSMALLINT* PrecisionPtr, SQLSMALLINT* ScalePtr, SQLSMALLINT* NullablePtr);
...
SQLRETURN SQLGetInfo(SQLHDBC ConnectionHandle, SQLUSMALLINT InfoType,
        SQLPOINTER InfoValue, SQLSMALLINT BufferLength, SQLSMALLINT* StringLengthPtr);

SQLRETURN SQLGetStmtAttr(SQLHSTMT StatementHandle, SQLINTEGER Attribute,
        SQLPOINTER Value, SQLINTEGER BufferLength, SQLINTEGER* StringLength);

SQLRETURN SQLGetStmtOption(SQLHSTMT StatementHandle, SQLUSMALLINT Option,
        SQLPOINTER Value);
...
SQLRETURN SQLSetConnectOption(SQLHDBC ConnectionHandle, SQLUSMALLINT Option,
        SQLULEN Value);
...
SQLRETURN SQLSetDescRec(SQLHDESC DescriptorHandle, SQLSMALLINT RecNumber,
        ...
        SQLSMALLINT Scale, SQLPOINTER Data, SQLLEN* StringLength, SQLLEN* Indicator);
...
SQLRETURN SQLSetStmtOption(SQLHSTMT StatementHandle, SQLUSMALLINT Option,
        SQLULEN Value);
...
SQLRETURN SQLStatistics(SQLHSTMT StatementHandle, SQLCHAR* CatalogName,
        SQLSMALLINT NameLength1, SQLCHAR* SchemaName, SQLSMALLINT NameLength2,
        SQLCHAR* TableName, SQLSMALLINT NameLength3, SQLUSMALLINT Unique,
...
SQLRETURN SQLTransact(SQLHENV EnvironmentHandle, SQLHDBC ConnectionHandle,
